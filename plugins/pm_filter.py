...
from info import ADMINS, ADMIN, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER, FILE_CHANNEL, FILE_FORWARD
...
@Client.on_message(filters.command(["apps_var", f"apps@{Config.BOT_USERNAME}"]))
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.message.edit_reply_markup(
                InlineKeyboardMarkup(
                    [
#                        [InlineKeyboardButton("⏹️ SET VAR ⏹️",
#                            callback_data=f"setvar_{data}")],
...
#    elif query.data.startswith("shutdown_"):
...
#            await query.answer(f"Shutting Down - {app[data]}", show_alert=True)
#        except Exception as e:
...
#    elif query.data.startswith("boot_"):
...
#            await query.answer(f"Booting - {app[data]}", show_alert=True)
#        except Exception as e:
#            await query.answer(f"Error: {e}", show_alert=True)
#           return
#        else:
...
#        await query.answer(f"{total}, {query.from_user.first_name}", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton(f'Sᴇʟᴇᴄᴛ ➢', 'update'),
                InlineKeyboardButton("ʟᴀɴɢᴜᴀɢᴇs", callback_data=f"languages#{key}"),
                InlineKeyboardButton("Sᴇᴀsᴏɴs", callback_data=f"seasons#{key}")
            ]
        )
